...
sudo sed -i "s/#shared_preload_libraries = ''/shared_preload_libraries = 'timescaledb'/g" /etc/postgresql/18/main/postgresql.conf
sudo sed -i "s/shared_preload_libraries = ''/shared_preload_libraries = 'timescaledb'/g" /etc/postgresql/18/main/postgresql.conf

# timescaledb-tune is highly recommended but sometimes prompts, using --quiet --yes

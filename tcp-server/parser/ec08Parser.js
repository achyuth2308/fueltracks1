 ...
 */
function parseEC08Buffer(buffer, imei) {
  const packets   = [];
  let   pos       = 0;
  ...
  while (pos < buffer.length) {
    ...
    try {
      ...
      switch (protocolNumber) {
        ...
        case 0x16:
        case 0x27:
          parsed = parseAlarm(info, serialNumber, imei, protocolNumber === 0x27, protocolNumber);
          break;
        case 0x94:
        ...
        case 0x8A:
          ...
          console.log(`[EC08] Time Check (0x8A) from ${imei || 'unknown'} — no response sent (per spec)`);
          parsed = { packetType: 'EC08_TIME_CHECK', imei: imei || null, serialNumber, rawPacketType: 0x8A };
          break;
        case 0x21:
          console.log(`[EC08] Command Response (0x21) from ${imei || 'unknown'}`);
          parsed = { packetType: 'EC08_COMMAND_RESPONSE', imei: imei || null, serialNumber, rawPacketType: 0x21 };
          break;
        case 0x80:
          console.log(`[EC08] Online Command (0x80) from ${imei || 'unknown'}`);
          parsed = { packetType: 'EC08_ONLINE_COMMAND', imei: imei || null, serialNumber, rawPacketType: 0x80 };
          break;
        default:
...
module.exports = {
  ...
  parseDateTime,
  parseTerminalInfo,
  crcItu,  // re-exported from ec08Crc via this module for convenience
};

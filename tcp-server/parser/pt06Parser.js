// ============================================================
// PT06 V5 / VL149 / GT800 BINARY PROTOCOL PARSER
// Pt06 protocol spec: binary TCP, 0x78 0x78 (normal) /
...
const { crcItu } = require('./pt06Crc');

// ------------------------------------------------------------
// Alarm type → canonical alert mapping
// Full table from Pt06 spec §4.1.ii
// ------------------------------------------------------------
const PT06_ALARM_MAP = {
...
/**
 * Decode an IMEI from 8-byte packed BCD in the Pt06 login packet.
...
function parseLogin(info, serialNumber) {
  if (info.length < 12) {
    throw new Error(`Pt06 Login: info too short (${info.length} bytes, expected 12)`);
  }
  ...
  if (!/^\d{14,16}$/.test(imei)) {
    throw new Error(`Pt06 Login: invalid IMEI decoded: '${imei}'`);
  }

  return {
    packetType:  'PT06_LOGIN',
    imei,
    modelCode,
    tzLang,
    serialNumber,
...
function parseHeartbeat(info, serialNumber, imei, rawPacketType = 0x13) {
  ...
  if (rawPacketType === 0x23) {
    if (info.length < 4) {
      throw new Error(`Pt06 Heartbeat: info too short (${info.length} bytes, expected 4)`);
    }
    ...
    if (info.length < 5) {
      throw new Error(`Pt06 Heartbeat: info too short (${info.length} bytes, expected 5)`);
    }
    termInfo    = parseTerminalInfo(info[0]);
    battLevel   = info[1];
    gsmLevel    = info[2];
...
function parseLocation(info, serialNumber, imei, rawPacketType = 0x22) {
  if (info.length < 29) {
    throw new Error(`Pt06 Location: info too short (${info.length} bytes, expected 29+)`);
  }
  ...
  return {
    ...
    isLive:         !isBuffered,
    odometer,
    fuel:           null,       // not reported in Pt06 location packets
    voltage,
...
function parseAlarm(info, serialNumber, imei, isMultiFence, rawPacketType) {
  if (info.length < 26) {
    throw new Error(`Pt06 Alarm: info too short (${info.length} bytes, expected 26+)`);
  }
  ...
  const alarmInfo  = PT06_ALARM_MAP[alarmCode] || {
    type: 'general',
  ...
  return {
    packetType:    isMultiFence ? 'PT06_ALARM_MULTI' : 'PT06_ALARM',
    imei:          imei || null,
    deviceTime,
    lat:           gps.lat,
    lng:           gps.lng,
...
function parseInfoTransmission(info, serialNumber, imei) {
  ...
  while (pos < info.length) {
    ...
    if (subType === 0x00) {
      ...
      console.debug(`[PT06] Skipping unhandled 0x94 sub-type 0x${subType.toString(16).padStart(2, '0')}`);
      ...
      console.debug(`[PT06] Unknown 0x94 sub-type 0x${subType.toString(16).padStart(2, '0')} — skipping remaining info bytes`);
      break;
    }
  }
...
/**
 * Parse all complete Pt06 frames from a binary Buffer.
 *
 * Handles both:
 ...
 */
function parsePt06Buffer(buffer, imei) {
  const packets   = [];
  let   pos       = 0;
  ...
  while (pos < buffer.length) {
    ...
    if (b0 === 0x78 && b1 === 0x78) {
      ...
      if (b0 >= 0x20 && b0 <= 0x7E) {
        ...
        if (crlfPos !== -1) {
          ...
          console.log(`[PT06] Interleaved ASCII Message: ${asciiStr}`);
          packets.push({
            packetType: 'PT06_ASCII_MESSAGE',
            rawString: asciiStr
          });
          pos = crlfPos + 2;
          continue;
      ...
      if (b0 !== 0x0A && b0 !== 0x0D) {
        // console.debug(`[PT06] Unexpected byte 0x${b0.toString(16)} at offset ${pos}, scanning forward`);
      }
      pos++;
      continue;
    }
    ...
    if (buffer[stopPos] !== 0x0D || buffer[stopPos + 1] !== 0x0A) {
      console.warn(`[PT06] Bad stop bytes at offset ${stopPos}: 0x${buffer[stopPos].toString(16)} 0x${buffer[stopPos + 1].toString(16)}, skipping frame`);
    ...
    if (computedCrc !== packetCrc) {
      console.warn(`[PT06] CRC mismatch: computed=0x${computedCrc.toString(16).toUpperCase()} packet=0x${packetCrc.toString(16).toUpperCase()} — discarding frame`);
    ...
    try {
      ...
      switch (protocolNumber) {
        ...
        case 0x8A:
          ...
          console.log(`[PT06] Time Check (0x8A) from ${imei || 'unknown'} — no response sent (per spec)`);
          parsed = { packetType: 'PT06_TIME_CHECK', imei: imei || null, serialNumber, rawPacketType: 0x8A };
          break;
        case 0x21:
          console.log(`[PT06] Command Response (0x21) from ${imei || 'unknown'}`);
          parsed = { packetType: 'PT06_COMMAND_RESPONSE', imei: imei || null, serialNumber, rawPacketType: 0x21 };
          break;
        case 0x80:
          console.log(`[PT06] Online Command (0x80) from ${imei || 'unknown'}`);
          parsed = { packetType: 'PT06_ONLINE_COMMAND', imei: imei || null, serialNumber, rawPacketType: 0x80 };
          break;
        default:
          console.warn(`[PT06] Unknown protocol number 0x${protocolNumber.toString(16).toUpperCase()} — logging raw frame`);
          parsed = {
            packetType: `PT06_0x${protocolNumber.toString(16).toUpperCase()}`,
            imei: imei || null,
            serialNumber,
            rawPacketType: protocolNumber,
            parsed: false,
            error: `Unknown Pt06 protocol number 0x${protocolNumber.toString(16).toUpperCase()}`
          };
      }
      ...
      console.error(`[PT06] Error parsing protocol 0x${protocolNumber.toString(16).toUpperCase()}:`, err.message);
    }
...
module.exports = {
  ...
  parseDateTime,
  parseTerminalInfo,
  crcItu,  // re-exported from pt06Crc via this module for convenience
};

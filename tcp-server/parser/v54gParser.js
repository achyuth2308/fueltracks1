// ============================================================
// EC08 V5 / VL149 / GT800 BINARY PROTOCOL PARSER
// EC08 protocol spec: binary TCP, 0x78 0x78 (normal) /
...
const { crcItu } = require('./ec08Crc');

// ------------------------------------------------------------
// Alarm type → canonical alert mapping
// Full table from EC08 spec §4.1.ii
// ------------------------------------------------------------
const EC08_ALARM_MAP = {
...
/**
 * Decode an IMEI from 8-byte packed BCD in the EC08 login packet.
...
function parseHeartbeat(info, serialNumber, imei, rawPacketType = 0x13) {
  ...
  if (rawPacketType === 0x23) {
    if (info.length < 4) {
      throw new Error(`EC08 Heartbeat: info too short (${info.length} bytes, expected 4)`);
    }
    ...
    if (info.length < 5) {
      throw new Error(`EC08 Heartbeat: info too short (${info.length} bytes, expected 5)`);
    }
    termInfo    = parseTerminalInfo(info[0]);
    battLevel   = info[1];
    gsmLevel    = info[2];
...
function parseAlarm(info, serialNumber, imei, isMultiFence, rawPacketType) {
  if (info.length < 26) {
    throw new Error(`EC08 Alarm: info too short (${info.length} bytes, expected 26+)`);
  }
  ...
  const alarmInfo  = EC08_ALARM_MAP[alarmCode] || {
    type: 'general',
  ...
  return {
    packetType:    isMultiFence ? 'EC08_ALARM_MULTI' : 'EC08_ALARM',
    imei:          imei || null,
    deviceTime,
    lat:           gps.lat,
    lng:           gps.lng,
...
function parseInfoTransmission(info, serialNumber, imei) {
  ...
  while (pos < info.length) {
    ...
    if (subType === 0x00) {
      ...
      console.debug(`[EC08] Skipping unhandled 0x94 sub-type 0x${subType.toString(16).padStart(2, '0')}`);
      ...
      console.debug(`[EC08] Unknown 0x94 sub-type 0x${subType.toString(16).padStart(2, '0')} — skipping remaining info bytes`);
      break;
    }
  }
...
/**
 * Parse all complete EC08 frames from a binary Buffer.
 *
 * Handles both:
 ...
 */
function parseV54GBuffer(buffer, sessionImei = null) {
  const packets   = [];
  let   pos       = 0;
  ...
  while (pos < buffer.length) {
    ...
    if (b0 === 0x78 && b1 === 0x78) {
      ...
      if (b0 >= 0x20 && b0 <= 0x7E) {
        ...
        if (crlfPos !== -1) {
          ...
          console.log(`[EC08] Interleaved ASCII Message: ${asciiStr}`);
          packets.push({
            packetType: 'EC08_ASCII_MESSAGE',
            rawString: asciiStr
          });
          pos = crlfPos + 2;
          continue;
      ...
      if (b0 !== 0x0A && b0 !== 0x0D) {
        // console.debug(`[EC08] Unexpected byte 0x${b0.toString(16)} at offset ${pos}, scanning forward`);
      }
      pos++;
      continue;
    }
    ...
    if (buffer[stopPos] !== 0x0D || buffer[stopPos + 1] !== 0x0A) {
      console.warn(`[EC08] Bad stop bytes at offset ${stopPos}: 0x${buffer[stopPos].toString(16)} 0x${buffer[stopPos + 1].toString(16)}, skipping frame`);
    ...
    if (computedCrc !== packetCrc) {
      console.warn(`[EC08] CRC mismatch: computed=0x${computedCrc.toString(16).toUpperCase()} packet=0x${packetCrc.toString(16).toUpperCase()} — discarding frame`);
    ...
    //   info ends at:   pos + frameSize - 6 (excludes serial, crc, stop)
    const infoStart = pos + 2 + lengthFieldSize + 1;
    const infoEnd   = pos + frameSize - 6;
    ...
    try {
      ...
      switch (protocolNumber) {
        ...
        case 0x13:
        case 0x23:
          parsed = parseHeartbeat(info, serialNumber, sessionImei, protocolNumber);
          break;
        case 0x12:
        case 0x22:
          parsed = parseLocation(info, serialNumber, sessionImei, protocolNumber);
          break;
        case 0x16:
        case 0x26:
        case 0x27:
          parsed = parseAlarm(info, serialNumber, sessionImei, protocolNumber === 0x27, protocolNumber);
          break;
        case 0x94:
          parsed = parseInfoTransmission(info, serialNumber, sessionImei);
          break;
        case 0x8A:
          ...
          console.log(`[EC08] Time Check (0x8A) from ${sessionImei || 'unknown'} — no response sent (per spec)`);
          parsed = { packetType: 'EC08_TIME_CHECK', imei: sessionImei || null, serialNumber, rawPacketType: 0x8A };
          break;
        case 0x21:
          console.log(`[EC08] Command Response (0x21) from ${sessionImei || 'unknown'}`);
          parsed = { packetType: 'EC08_COMMAND_RESPONSE', imei: sessionImei || null, serialNumber, rawPacketType: 0x21 };
          break;
        case 0x80:
          console.log(`[EC08] Online Command (0x80) from ${sessionImei || 'unknown'}`);
          parsed = { packetType: 'EC08_ONLINE_COMMAND', imei: sessionImei || null, serialNumber, rawPacketType: 0x80 };
          break;
        default:
          console.warn(`[EC08] Unknown protocol number 0x${protocolNumber.toString(16).toUpperCase()} — logging raw frame`);
          parsed = {
            packetType: `EC08_0x${protocolNumber.toString(16).toUpperCase()}`,
            imei: sessionImei || null,
            serialNumber,
            rawPacketType: protocolNumber,
            parsed: false,
            error: `Unknown EC08 protocol number 0x${protocolNumber.toString(16).toUpperCase()}`
          };
      }
      ...
      console.error(`[EC08] Error parsing protocol 0x${protocolNumber.toString(16).toUpperCase()}:`, err.message);
    }

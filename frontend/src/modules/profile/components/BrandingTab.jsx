...
const BrandingTab = ({ profile, onUpload }) => {
  ...
  const renderUploadBox = (title, type, currentUrl, inputRef) => (
    <div className="border-2 border-dashed border-slate-300 rounded-2xl p-6 bg-white flex flex-col hover:bg-slate-100 transition-colors cursor-pointer items-center justify-center text-center">
      <h4 className="text-md font-medium mb-4" style={{ color: '#000000', margin: '0 0 16px 0' }}>{title}</h4>
      ...
      {currentUrl ? (
        ...
        <div className="mb-4 w-32 h-32 rounded-md bg-orange-50 flex items-center justify-center" style={{ color: '#000000' }}>
      ...
      <p className="text-xs mt-3" style={{ color: '#000000', margin: '12px 0 0 0' }}>PNG, JPG, SVG up to 5MB</p>
    </div>
  );

  return (
    <div className="bg-white p-6 sm:p-10 rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-slate-200">
      <h2 style={{ fontSize: "18px", fontWeight: 700, color: "#000000", margin: 0, letterSpacing: "-0.01em", marginBottom: "24px" }}>Branding Configuration</h2>

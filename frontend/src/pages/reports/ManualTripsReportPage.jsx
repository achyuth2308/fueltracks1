...
const ManualTripsReportPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const trips = await tripApi.list(params);
      setData(trips || []);
    } catch (err) {
      console.error(err);
  ...
  const getExportData = () => {
    return data.map(row => ({
      'Trip Name': row.name || 'Unnamed',
      'Status': row.status || 'N/A',
      'Start Time': row.start_time ? formatLocalTime(row.start_time) : 'N/A',
      'Start Loc': row.origin || ((row.start_lat && row.start_lng) ? `Lat: ${row.start_lat}, Lng: ${row.start_lng}` : 'N/A'),
      'End Time': row.end_time ? formatLocalTime(row.end_time) : 'N/A',
      'End Loc': row.destination || ((row.end_lat && row.end_lng) ? `Lat: ${row.end_lat}, Lng: ${row.end_lng}` : 'N/A'),
      ...
      <div style={{ background: '#FFFFFF', borderRadius: '16px', border: '1px solid #E2E8F0', flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#FAFAFA' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button onClick={() => exportToPDF(columns, getExportData(), 'Manual Trips Report', 'manual_trips_report')} disabled={data.length === 0} style={{ padding: '8px 16px', borderRadius: '8px', background: '#FFFFFF', border: '1px solid #CBD5E1', color: '#475569', fontSize: '13px', fontWeight: 600, cursor: data.length ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: '6px', opacity: data.length ? 1 : 0.5 }}>
              <FileText size={16} color="#DC2626" /> PDF
            </button>
            <button onClick={() => exportToExcel(getExportData(), 'manual_trips_report')} disabled={data.length === 0} style={{ padding: '8px 16px', borderRadius: '8px', background: '#FFFFFF', border: '1px solid #CBD5E1', color: '#475569', fontSize: '13px', fontWeight: 600, cursor: data.length ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: '6px', opacity: data.length ? 1 : 0.5 }}>
              <Download size={16} color="#10B981" /> Excel
            </button>
            <button onClick={() => exportToCSV(getExportData(), 'manual_trips_report')} disabled={data.length === 0} style={{ padding: '8px 16px', borderRadius: '8px', background: '#FFFFFF', border: '1px solid #CBD5E1', color: '#475569', fontSize: '13px', fontWeight: 600, cursor: data.length ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: '6px', opacity: data.length ? 1 : 0.5 }}>
            ...
            <tbody>
              {data.length === 0 ? (
                ...
                <tr key={idx} style={{ borderBottom: '1px solid #F1F5F9' }}>
                  <td style={{ padding: '10px 16px', fontSize: '13px', color: '#475569', fontWeight: 600 }}>{row.name}</td>
                  <td style={{ padding: '10px 16px', fontSize: '13px', color: '#475569' }}><span style={{ padding: '4px 8px', borderRadius: '4px', background: row.status === 'in_progress' ? '#FEF08A' : row.status === 'completed' ? '#BBF7D0' : '#E2E8F0' }}>{row.status}</span></td>
                  <td style={{ padding: '10px 16px', fontSize: '13px', color: '#475569' }}>{row.start_time ? formatLocalTime(row.start_time) : '-'}</td>
                  {row.origin ? <td style={{ padding: '10px 16px', fontSize: '13px', color: '#475569' }}>{row.origin}</td> : <AddressCell lat={row.start_lat} lng={row.start_lng} />}
                  <td style={{ padding: '10px 16px', fontSize: '13px', color: '#475569' }}>{row.end_time ? formatLocalTime(row.end_time) : '-'}</td>
                  {row.destination ? <td style={{ padding: '10px 16px', fontSize: '13px', color: '#475569' }}>{row.destination}</td> : <AddressCell lat={row.end_lat} lng={row.end_lng} />}
                  <td style={{ padding: '10px 16px', fontSize: '13px', color: '#475569' }}>{formatDuration(row.duration_secs || row.duration_seconds)}</td>
...
export default ManualTripsReportPage;

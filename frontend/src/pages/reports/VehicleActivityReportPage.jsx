...
import { ArrowLeft, Download, Search, Loader2, Activity, Filter, FileText } from 'lucide-react';
import axiosInstance from '../../api/axios';
import { exportToExcel, exportToPDF, exportToCSV } from '../../utils/exportUtils';
import * as vehicleApi from '../../api/vehicleApi';

const VehicleActivityReportPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.get(`/api/reports/activity?${params.toString()}`);
      if(res.data.success) {
  ...
  const getExportData = () => {
    return data.map(row => ({
      ...
      'Stopped (hrs)': (row.stopped_seconds / 3600).toFixed(2),
      'Distance (km)': row.distance_travelled || 0,
  ...
  return (
    <div style={{ padding: '32px', background: 'linear-gradient(to bottom, #FFF7ED 0%, #FFF7ED 50%, #F8FAFC 50%, #F8FAFC 100%)', minHeight: '100%', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '32px' }}>
        ...
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 800, color: '#111827', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Activity size={24} color="#059669" /> Vehicle Activity Report
          </h1>
          <p style={{ fontSize: '14px', color: '#64748B', margin: '4px 0 0 0' }}>Breakdown of running, idling, stopped, and offline hours.</p>
        </div>
      </div>
      ...
      <div style={{ background: '#FFFFFF', padding: '24px', borderRadius: '16px', border: '1px solid #E2E8F0', boxShadow: '0 4px 6px rgba(0,0,0,0.02)', marginBottom: '24px', display: 'flex', gap: '24px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        <button onClick={handleGenerate} disabled={loading} style={{ padding: '12px 24px', borderRadius: '10px', background: '#059669', color: '#FFF', border: 'none', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 12px rgba(5,150,105,0.2)' }}>
      ...
      <div style={{ background: '#FFFFFF', borderRadius: '16px', border: '1px solid #E2E8F0', flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '16px 24px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#FAFAFA' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button onClick={() => exportToPDF(columns, getExportData(), 'Vehicle Activity Report', 'activity_report')} disabled={data.length === 0} style={{ padding: '8px 16px', borderRadius: '8px', background: '#FFFFFF', border: '1px solid #CBD5E1', color: '#475569', fontSize: '13px', fontWeight: 600, cursor: data.length ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: '6px', opacity: data.length ? 1 : 0.5 }}>
              <FileText size={16} color="#DC2626" /> PDF
            </button>
            <button onClick={() => exportToExcel(getExportData(), 'activity_report')} disabled={data.length === 0} style={{ padding: '8px 16px', borderRadius: '8px', background: '#FFFFFF', border: '1px solid #CBD5E1', color: '#475569', fontSize: '13px', fontWeight: 600, cursor: data.length ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: '6px', opacity: data.length ? 1 : 0.5 }}>
              <Download size={16} color="#10B981" /> Excel
            </button>
            <button onClick={() => exportToCSV(getExportData(), 'activity_report')} disabled={data.length === 0} style={{ padding: '8px 16px', borderRadius: '8px', background: '#FFFFFF', border: '1px solid #CBD5E1', color: '#475569', fontSize: '13px', fontWeight: 600, cursor: data.length ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: '6px', opacity: data.length ? 1 : 0.5 }}>
            ...
            <tbody>
              {data.length === 0 ? (
                ...
                <tr key={idx} style={{ borderBottom: '1px solid #F1F5F9' }}>
                  ...
                  <td style={{ padding: '14px 24px', fontSize: '13px', color: '#10B981', fontWeight: 600 }}>{(row.running_seconds / 3600).toFixed(2)} h</td>
                  <td style={{ padding: '14px 24px', fontSize: '13px', color: '#F59E0B', fontWeight: 600 }}>{(row.idle_seconds / 3600).toFixed(2)} h</td>
                  <td style={{ padding: '14px 24px', fontSize: '13px', color: '#64748B', fontWeight: 600 }}>{(row.stopped_seconds / 3600).toFixed(2)} h</td>
                  <td style={{ padding: '14px 24px', fontSize: '13px', color: '#475569' }}>{row.distance_travelled || 0}</td>
                  <td style={{ padding: '14px 24px', fontSize: '13px', color: '#475569' }}>{row.trip_count || 0}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default VehicleActivityReportPage;

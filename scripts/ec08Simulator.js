...
//   Protocol 3: Ec08 Binary → Port 5002 (binary, 0x78/0x79 frames)
...
//   node scripts/deviceSimulator.js ec08       (Ec08 only)
...
const { crcItu } = require('../tcp-server/parser/ec08Crc');
...
const PORTS = {
  ...
  EC08: parseInt(process.env.EC08_TCP_PORT) || 5002,
  ...
  AIS140_DEVICES.forEach((device) => {
    ...
    client.connect(PORTS.AIS140, TCP_HOST, () => {
      ...
      intervalId = setInterval(() => {
        ...
        if (tick % 4 === 0 && device.imei === '867123456789012') {
...
// ============================================================
// EC08 BINARY SIMULATOR  (port 5002)
// ============================================================

const EC08_DEVICES = [
  {
    imei: '867123456789012',
    imeiBytes: [0x86, 0x71, 0x23, 0x45, 0x67, 0x89, 0x01, 0x20],
    lat: 17.345378,
    lng: 78.523923,
    speed: 0,
    ignition: false,
  ...
  {
    imei: '867123456789013',
    imeiBytes: [0x86, 0x71, 0x23, 0x45, 0x67, 0x89, 0x01, 0x30],
    lat: 17.445378,
    lng: 78.623923,
    speed: 55,
    ignition: true,
 ...
 * e.g. "867123456789012" → pad to 16 digits "0867123456789012"
 ...
 */
function buildEc08Frame(protocolNum, infoBytes, serialNum) {
...
function buildEc08ExtendedFrame(protocolNum, infoBytes, serialNum) {
  const info = Buffer.isBuffer(infoBytes) ? infoBytes : Buffer.from(infoBytes);
  const pktLen = 1 + info.length + 2 + 2;  // protocol + info + serial + crc
...
function makeEc08Login(device) {
  ...
  return buildEc08Frame(0x01, info, nextSerial(device));
}

/** Build Heartbeat packet (0x13) */
function makeEc08Heartbeat(device) {
  ...
  return buildEc08Frame(0x13, info, nextSerial(device));
}

/** Build Info packet (0x94) for External Voltage */
function makeEc08Info(device) {
  ...
  return buildEc08ExtendedFrame(0x94, voltageBytes, nextSerial(device));
}

/** Build Location packet (0x12) */
function makeEc08Location(device) {
  const now = utcNow();
  const pad = n => n & 0xFF;
  ...
  return buildEc08Frame(0x12, info, nextSerial(device));
}

/** Build Alarm packet (0x16) — overspeed alarm */
function makeEc08Alarm(device, alarmCode = 0x06) {
  const now = utcNow();
  const pad = n => n & 0xFF;
  ...
  return buildEc08Frame(0x16, info, nextSerial(device));
}

function updateEc08State(d) {
...
function startEc08Simulator() {
  log('EC08', `Starting — targeting ${TCP_HOST}:${5007}`);

  EC08_DEVICES.forEach((device) => {
    const client = new net.Socket();
    let heartbeatId = null;
    let locationId = null;
    let tick = 0;

    client.connect(5007, TCP_HOST, () => {
      log('EC08', `${C.ok}Connected${C.reset} IMEI=${device.imei}`);

      // Step 1: Send login immediately (CRITICAL — device reboots if no ACK within 5s)
      const loginFrame = makeEc08Login(device);
      client.write(loginFrame);
      log('EC08', `${C.ok}→ LOGIN (0x01)${C.reset} IMEI=${device.imei} bytes=[${loginFrame.toString('hex').toUpperCase().match(/../g).join(' ')}]`);

      // Step 2: Heartbeat every 30s
      heartbeatId = setInterval(() => {
        const hbFrame = makeEc08Heartbeat(device);
        client.write(hbFrame);
        log('EC08', `${C.ok}→ HEARTBEAT (0x13)${C.reset} ign=${device.ignition} batt=80%`);
      }, 30000);

      // Step 3: Location every 10s
      locationId = setInterval(() => {
        tick++;
        updateEc08State(device);

        const locFrame = makeEc08Location(device);
        client.write(locFrame);
        log('EC08', `${C.ok}→ LOCATION (0x12)${C.reset} lat=${device.lat.toFixed(6)} lng=${device.lng.toFixed(6)} speed=${device.speed} ign=${device.ignition} isLive=true`);

        // Send Info packet immediately after Location
        const infoFrame = makeEc08Info(device);
        client.write(infoFrame);
        log('EC08', `${C.ok}→ INFO (0x94)${C.reset} external voltage`);

        // Send overspeed alarm when speed > 80
        if (device.speed > 80 && device.imei === '867123456789012') {
          const alarmFrame = makeEc08Alarm(device, 0x06);  // 0x06 = overspeed
          client.write(alarmFrame);
          log('EC08', `${C.ok}→ ALARM (0x16)${C.reset} alarmCode=0x06 (Overspeed) speed=${device.speed}`);
        }

        // Send SOS alarm once per full stage cycle (stage 3)
        if (device.stage === 3 && device.stageCtr === 1 && device.imei === '867123456789012') {
          const sosFrame = makeEc08Alarm(device, 0x01);  // 0x01 = SOS
          client.write(sosFrame);
          log('EC08', `${C.ok}→ ALARM (0x16)${C.reset} alarmCode=0x01 (SOS Alarm)`);
        }
      }, 10000);
    });
    ...
    client.on('data', (data) => {
      ...
      while (offset + 4 <= data.length) {
        ...
        const protoNames = { 0x01: 'LOGIN ACK', 0x13: 'HEARTBEAT ACK', 0x16: 'ALARM ACK', 0x27: 'ALARM ACK' };
        ...
        log('EC08', `${C.ack}← ${protoName}${C.reset} bytes=[${frameHex}]`);
        offset = frameEnd;
      }
    });

    client.on('error', (err) => {
      log('EC08', `${C.err}ERROR${C.reset} ${device.imei}: ${err.message}`);
      if (heartbeatId) clearInterval(heartbeatId);
      if (locationId) clearInterval(locationId);
    });

    client.on('close', () => {
      log('EC08', `${C.err}Disconnected${C.reset} ${device.imei}`);
      if (heartbeatId) clearInterval(heartbeatId);
      if (locationId) clearInterval(locationId);
    });
  });
...
console.log(`${C.bold}║  ${C.ec08}Ec08     ${C.reset}${C.bold}→  port ${5007}  (Binary 0x78/0x79)         ║${C.reset}`);
